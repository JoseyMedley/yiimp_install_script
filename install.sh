...
echo -e "$GREEN Install Yiimp on Debian 12 running Nginx, MariaDB, and php8.3 $COL_RESET"
echo -e "$GREEN************************************************************************$COL_RESET"
echo
sleep 3
...
sudo apt -y install libgmp3-dev default-libmysqlclient-dev libcurl4-gnutls-dev libkrb5-dev libldap2-dev libidn11-dev libgnutls28-dev \
...
cd $HOME/pool.medleytechnologies.com/blocknotify
sudo sed -i 's/tu8tu5/'$blckntifypass'/' blocknotify.cpp
make -j$((`nproc`+1))

# Compile Stratum
cd $HOME/pool.medleytechnologies.com/stratum/
...
cd $HOME/pool.medleytechnologies.com/stratum/
if [[ ("$BTC" == "y" || "$BTC" == "Y") ]]; then
sudo sed -i 's/CFLAGS += -DNO_EXCHANGE/#CFLAGS += -DNO_EXCHANGE/' $HOME/pool.medleytechnologies.com/stratum/Makefile
fi
make -j$((`nproc`+1))

# Copy Files (Blocknotify,iniparser,Stratum)
cd $HOME/pool.medleytechnologies.com
sudo sed -i 's/AdminRights/'AdminPanel'/' $HOME/pool.medleytechnologies.com/web/yaamp/modules/site/SiteController.php
sudo cp -r $HOME/pool.medleytechnologies.com/web /var/
...
cd $HOME/pool.medleytechnologies.com
sudo cp -r $HOME/pool.medleytechnologies.com/bin/. /bin/
sudo cp -r $HOME/pool.medleytechnologies.com/blocknotify/blocknotify /usr/bin/
sudo cp -r $HOME/pool.medleytechnologies.com/blocknotify/blocknotify /var/stratum/
